...
[![Live preview](https://img.shields.io/badge/live-Cloudflare-F38020?logo=cloudflare&logoColor=white)](https://luckycommons.luckycommons.workers.dev)
...
distributions, reserves, or verifiable `$LUCK` buy-and-burn activity.
...
- Product preview: [luckycommons.luckycommons.workers.dev](https://luckycommons.luckycommons.workers.dev)
...
| [Protocol reference](docs/PROTOCOL.md) | EVM and Solana state transitions, licensing, revenue, receipts, and `$LUCK` behavior |
...
NFT deposit -> pool position -> published prize eligibility
             ...
             optional rights review
              ...
              paid media licenses
                       |
       prizes / member drops / reserves / $LUCK burn
```
...
The current `$LUCK` contract is a fixed-supply EVM token. Deployments on
different EVM chains are independent, and the Solana program does not yet mint
an SPL `$LUCK`. The repository therefore does **not** claim a synchronized
cross-chain token supply. See [Protocol reference](docs/PROTOCOL.md#luck-token-model).
...
The deployment script creates the `$LUCK` token, vault, and licensing contracts,
